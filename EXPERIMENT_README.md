...
For the current vertical slice, nftables sends the first outbound IPv4 or IPv6 TCP SYN and the first datagram of each ordinary UDP flow to a small `libnetfilter_queue` helper. The helper holds that exact packet while TypeScript requests a decision. `ALLOW` repeats the packet with a trusted mark that nftables copies to its conntrack entry; only that flow is then forwarded by `slirp4netns`. `DENY` repeats it with a reject mark, producing a TCP reset or UDP rejection without a corresponding host-side connection or datagram. A pending conntrack mark drops TCP retransmissions and additional ordinary UDP datagrams while the decision is open.

Worker-only resolver and NSS files force ordinary hostname clients away from host systemd-resolved IPC and onto a gated DNS proxy. Every outbound UDP DNS query is held and validated before the trusted proxy sends it to the host resolver. The proxy validates the response, follows answer-section CNAME attribution, replaces approved A and AAAA answers with worker-scoped synthetic addresses, and installs namespace DNAT only after each lease is recorded. A later connection to that synthetic address is therefore authoritatively identified by the requested hostname while retaining its selected real address for logging and future address policy. Shared real IPs receive distinct synthetic leases for distinct hostnames. Unknown or expired synthetic destinations fail closed instead of becoming literal-IP targets. Direct DNS to other resolvers and TCP DNS fallback currently fail closed.

The enforcement core still emits a verdict and detailed audit event for every DNS query and TCP or UDP flow, including flows on the worker network namespace's loopback interface. A per-command policy projector independently controls user-approval granularity. By default, operation and address-family distinctions are disabled: approving a hostname DNS request authorizes that exact hostname across DNS, TCP, UDP, IPv4, IPv6, and destination ports for the remainder of that `bash-network` invocation. Literal-IP flows begin at an exact address and port; literal 127/8 and `::1` flows normalize to the `localhost` policy identity. The session-owned `PilotRuntimeConfig.networkPolicyGranularity` object controls whether DNS/TCP/UDP operations and actual IPv4/IPv6 flows are distinguished; both values currently default to `false`. `PilotSessionRuntime` owns a validated `PilotRuntimeConfig` instance supplied through its constructor options. It is intentionally in-memory only, leaving file loading and saving for a later JSON-backed configuration boundary. Resolver transport family is diagnostic only. Raw logs remain detailed in every mode. `localhost` still names the private worker namespace, not the host namespace, so this does not restore transparent access to host-loopback-only services.

IPv4-mapped IPv6 policy normalization remains future work. Active-flow revocation, TCP DNS proxying, persistent network policy, and integration with the root-wide FUSE worker remain unimplemented. Filesystem operations and pathname local IPC are deliberately not mediated by this tool. Because unprivileged network namespaces require a user namespace, the worker retains the host UID and primary GID but cannot necessarily retain every supplementary group; abstract Unix sockets are also network-namespace scoped.
...
- `src/runtime/PilotSessionRuntime.ts` — session-owned policy database, policy runtime, decision-flow manager, and runtime config.
- `src/runtime/PilotRuntimeConfig.ts` — validated in-memory configuration boundary, ready for later JSON loading and saving.
...
The network path holds the original TCP SYN or initial UDP datagram in NFQUEUE and binds approval to that packet's conntrack generation rather than a timed tuple. If an initiating TCP socket closes while approval is pending, approving still releases the original queued SYN; a replacement socket reusing the tuple receives a separate decision. IPv6 packets with extension headers currently fail closed. The implementation remains an initial reference-monitor seam, not yet the DNS identity, active revocation, resource-control, or durable policy model required by [`NETWORK-MVP.md`](./NETWORK-MVP.md). Network effects delegated to an existing host service through local IPC are intentionally outside this tool's policy boundary.
