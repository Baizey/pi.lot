...
The built-in `bash`, `read`, `edit`, and `write` tools, all subagent tools, and dynamically registered MCP tools use Pi's native `Ctrl+O` expansion state as a global density toggle. Collapsed tools show a minimal title-only view and hide result text, including errors; expanded tools show bounded argument and result previews. A row-local full-display flag is implemented for future per-tool interaction, but no user action sets it yet. Full Read and Write views retain Pi's native text and syntax highlighting, Read retains native image behavior, and full Edit views retain Pi's native diff preview. Pi's `multi_tool_use.parallel` orchestration tool is not overridden.
