...
One `PilotSessionRuntime` is created when Pi starts a session and closed on session shutdown. It owns validated runtime configuration, the policy database, session policy state, and the shared UI decision-flow manager; the FUSE Bash adapter only consumes those services. The Bash override is installed before runtime startup and fails closed if the session runtime is unavailable.

The built-in `bash`, `read`, `edit`, and `write` tools, all subagent tools, and dynamically registered MCP tools use Pi's native `Ctrl+O` expansion state as a global density toggle. Collapsed tools show a minimal title-only view and hide result text, including errors; expanded tools show bounded argument and result previews. The bounded `/view-full-tool` selector toggles row-local full views; Enter applies a toggle without closing the list, and Escape closes it. All of these tools self-render a one-cell status rail, coloring only the first padding cell of each visible line for pending, success, or error state instead of painting a full-width box. Full Read and Write views retain Pi's native text and syntax highlighting, Read retains native image behavior, and full Edit views retain Pi's native diff preview. Pi's `multi_tool_use.parallel` orchestration tool is not overridden.
